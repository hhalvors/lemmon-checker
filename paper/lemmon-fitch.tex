...
construction translating any Lemmon proof by way of its unfolding, and
conjecture that it is total. The construction is implemented, and its output
has been machine-checked on a corpus of regression cases together with sixteen
proofs photographed from student work; we set out precisely what remains to be
proved, and identify one place---the side condition on universal
introduction---where the two systems may not agree.
\end{abstract}
...
sharing of lines, and gives the construction that translates by unfolding.
Section~\ref{sec:open} states what remains to be proved, and three questions we
have not settled. Section~\ref{sec:machine} reports the machine verification, and
...
The construction of Definition~\ref{def:construction} yields, for every
correct Lemmon proof, a well-formed and correct Fitch proof with the same
conclusion, resting on premises drawn from among its own.
\end{conjecture}

The reason to expect this is that neither obstruction can arise. A derivation
is a tree and so carries no linear order for a discharge order to violate: a
node that discharges closes exactly the subproof its own subtree opened, and
nesting is automatic. And no node is shared, so no citation crosses from one
branch to another, and no line can be stranded by the closing of a subproof it
does not belong to.

That argument is not a proof, and we are careful to claim no more than we
have. It establishes that the two obstructions of
Section~\ref{sec:obstructions} do not occur; it does not establish that the
output satisfies the conditions of Definition~\ref{def:fitch}, nor that each
rule application in it is correct. Section~\ref{sec:open} sets out what a
proof would require.
...
introduction is the difficulty of the next subsection.
\end{enumerate}

\subsection{The eigenvariable problem}\label{sec:eigen}

Universal introduction carries a side condition, and the two systems state it
against different things.

In the Lemmon system, $\forall x\varphi(x)$ may be inferred from $\varphi(a)$
at line $n$ provided the name $a$ occurs in no assumption in $\Gamma(n)$---in
the assumptions the line \emph{depends on}. In the Fitch system, the same
inference is permitted provided $a$ occurs in no assumption \emph{in scope}.
By Proposition~\ref{prop:minimal} the scope may properly include the
dependency set. The Fitch condition is therefore the stronger of the two.

\begin{problem}\label{prob:eigen}
Let $L$ be a correct Lemmon proof and $F$ its image under
Definition~\ref{def:construction}. Suppose $L$ applies universal introduction
at some line, on a name $a$. Must $a$ be absent from every assumption in the
scope of the corresponding line of $F$?
\end{problem}

We do not know. Nothing in the construction places a line at the depth of its
dependencies: a line is emitted inside the subproof opened by a discharge that
is an ancestor of it in the derivation, and being a descendant of a discharge
does not require depending on what it discharges. So there is no evident
reason why the stronger condition should follow from the weaker, and a
counterexample would show Conjecture~\ref{conj:total} false as stated---not
merely unproved, since the Fitch proof produced would not be correct.

Two remarks may help whoever takes this up. First, if the answer is negative,
the natural repair is to place each line at the depth of the innermost
assumption in its dependency set rather than wherever the traversal puts it,
which would make scope and dependency coincide and the two side conditions
agree. Whether that placement is always available is
Conjecture~\ref{conj:noduplication} below, so the two questions are connected.
Second, the question is decidable for any given proof, and cheaply: extend the
well-formedness check of Section~\ref{sec:machine} to test the eigenvariable
condition against scope, and run it over a corpus. A counterexample, if one
exists among ordinary proofs, would announce itself.
...
(Conjecture~\ref{conj:total}, modulo Problem~\ref{prob:eigen} of
Section~\ref{sec:eigen}). Composing in one order returns the original
